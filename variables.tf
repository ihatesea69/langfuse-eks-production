...
variable "domain" {
  description = "Full domain name for Langfuse (e.g., langfuse.abc.com)"
  type        = string
}

variable "route53_zone_name" {
  description = "Name of the EXISTING Route53 hosted zone (e.g., abc.com). The Langfuse DNS record will be added to this zone."
  type        = string
}
...
variable "postgres_max_capacity" {
  ...
  type        = number
  default     = 8.0 # Production: scale up to 8 ACU (~16GB RAM)
}
...
variable "langfuse_web_replicas" {
  ...
  type        = number
  default     = 2 # HA: minimum 2 replicas
  validation {
...
variable "redis_at_rest_encryption" {
  ...
  type        = bool
  default     = true # Production: always encrypt at rest
}

variable "redis_multi_az" {
  description = "Whether Multi-AZ is enabled for the Redis cluster"
  type        = bool
  default     = true # Production: Multi-AZ for automatic failover
}
